...
namespace CoolParking.BL.Tests
{
    public class ParkingServiceTests : IDisposable
    {
        ...
        [Fact]
        public void GetFreePlaces_WhenOneCar_Then9()
        {
            var parkingService = new ParkingService(_withdrawTimer, _logTimer, _logService);
            var vehicle = new Vehicle("AA-0001-AA", VehicleType.Truck, 100);
            parkingService.AddVehicle(vehicle);
            Assert.Equal(9, _parkingService.GetFreePlaces());
        }
        ...
        [Fact]
        public void RegularWithdraw_WhenVehiclePositiveBalance_TaxIsWithdrawedFromVehicleToParkingBalance()
        {
            var vehicle = new Vehicle("AA-0001-AA", VehicleType.Truck, 100);
            _parkingService.AddVehicle(vehicle);
